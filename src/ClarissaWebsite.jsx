  ...
  return (
    <div style={{ background: "#F9F6F2", color: "#0D0D0D", minHeight: "100vh", fontFamily: "inherit", overflowX: "hidden" }}>
      ...
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1.25rem 2.5rem", borderBottom: "0.5px solid rgba(0,0,0,0.09)", background: "rgba(249,246,242,0.92)", backdropFilter: "blur(10px)", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <button style={{ background: "#5C2D82", color: "#fff", border: "none", padding: "10px 22px", borderRadius: "3px", fontSize: "12px", letterSpacing: "0.08em", textTransform: "uppercase", cursor: "pointer" }}>Schedule</button>
      </nav>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "1.1fr 0.9fr", minHeight: "88vh" }}>
        <motion.div initial="hidden" animate="visible" variants={stagger} style={{ padding: "5rem 3rem 4rem 2.5rem", display: "flex", flexDirection: "column", justifyContent: "center", borderRight: "0.5px solid rgba(0,0,0,0.09)" }}>
          ...
          <motion.div variants={fadeUp} style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
            <button onClick={() => window.location.href='#booking'} style={{ background: "#5C2D82", color: "#fff", padding: "14px 30px", fontSize: "12px", letterSpacing: "0.1em", textTransform: "uppercase", border: "none", borderRadius: "3px", cursor: "pointer", display: "flex", alignItems: "center", gap: "8px" }}>
              Book a consultation <ArrowRight size={14} />
            </button>
            <button onClick={() => window.location.href='#services'} style={{ background: "transparent", color: "#0D0D0D", padding: "14px 30px", fontSize: "12px", letterSpacing: "0.1em", textTransform: "uppercase", border: "0.5px solid rgba(0,0,0,0.25)", borderRadius: "3px", cursor: "pointer" }}>Explore services</button>
          </motion.div>
        </motion.div>
      ...
      <section id="services" style={{ background: "#1A0F24", padding: "6rem 2.5rem" }}>
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={stagger}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "1px", background: "rgba(255,255,255,0.07)", marginBottom: "3rem" }}>
            {[
              { title: "Private Educational Consulting", price: "$375", desc: "Personalized 1:1 consulting for families navigating complex educational decisions, learning differences, or academic transitions." },
              { title: "Academic Tutoring", price: "$337.50", desc: "Expert subject-matter support tailored to your learner's pace, style, and goals — from foundational skills to advanced coursework." },
              { title: "Intensive Academic Intervention", price: "$412.50", desc: "Targeted, high-impact sessions for students who need accelerated support in literacy, math, or executive functioning." },
              { title: "Homeschool Planning & Curriculum Design", price: "$450", desc: "Custom curriculum architecture and learning plans for families building rigorous, joyful homeschool programs." },
              { title: "Educational Advocacy & Parent Strategy", price: "$375", desc: "Empowering parents to navigate IEPs, 504 plans, school systems, and educational rights with confidence and clarity." },
              { title: "Dissertation & Research Consulting", price: "$375", desc: "Expert guidance for doctoral students and researchers navigating qualitative methodology, data analysis, and academic writing." },
              { title: "Educational Policy & EdTech Consulting", price: "$525", desc: "Strategic consulting for organizations, institutions, and founders at the intersection of education, policy, and technology." },
              { title: "School / Organizational Consulting", price: "$525", desc: "Systems-level consulting for schools and educational organizations seeking to improve outcomes, equity, and culture." },
            ].map((service, index) => (
              <motion.div key={service.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.1 }} transition={{ duration: 0.5, delay: index * 0.05 }}
                ...
                <div style={{ fontSize: "18px", fontWeight: 500, color: "#9B6BBD", marginBottom: "6px" }}>{service.price}<span style={{ fontSize: "11px", color: "rgba(255,255,255,0.3)", marginLeft: "6px" }}>/ session</span></div>
